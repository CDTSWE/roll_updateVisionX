...
BEGIN
    ...
    FOR i IN 0 .. jsonb_array_length(patch_operations) - 1 LOOP
        ...
        CASE op_type
            WHEN 'replace' THEN
                ...
                ELSIF clean_path = 'code' THEN
                    updated_code := value;
                ELSIF clean_path = 'priority' THEN
                    updated_priority := (op->>'value')::priority;
                ELSIF clean_path = 'reason' THEN
                    updated_reason := value;
                ELSIF clean_path = 'occurrenceDateTime' THEN
                    updated_occurrence_datetime := (op->>'value')::timestamp;
                ELSIF clean_path = 'insurance' THEN
                    updated_insurance := op->>'value';
                ELSIF clean_path = 'locationCode' THEN
                    updated_location := op->>'value';
                 ELSIF clean_path = 'bookmarks' THEN
                    updated_bookmarks := value;
                ELSIF clean_path = 'isExported' THEN
                    updated_is_exported := value;
                ELSIF clean_path = 'note' THEN
                    updated_note := op->>'value';
                ELSIF clean_path = 'subject' THEN
                    updated_subject := op->>'value';
                ELSIF clean_path = 'subjectDisplay' THEN
                    updated_subject_display := op->>'value';
                ELSIF clean_path = 'requester' THEN
                    updated_requester := op->>'value';
                ELSIF clean_path = 'requesterDisplay' THEN
                    updated_requester_display := op->>'value';
                ELSIF clean_path = 'performer' THEN
                    updated_performer := op->>'value';
                ELSIF clean_path = 'performerDisplay' THEN
                    updated_performer_display := op->>'value';
                ELSE
            ...
            WHEN 'remove' THEN
                ...
                ELSIF clean_path = 'insurance' THEN
                    updated_insurance := NULL;
                ELSIF clean_path = 'locationCode' THEN
                    updated_location := NULL;
                ELSIF clean_path = 'bookmarks' THEN
                    updated_bookmarks := NULL;
                ELSIF clean_path = 'isExported' THEN
                    updated_is_exported := NULL;
                ELSIF clean_path = 'note' THEN
                    updated_note := NULL;
                ELSIF clean_path = 'subject' THEN
                    updated_subject := NULL;
                ELSIF clean_path = 'subjectDisplay' THEN
                    updated_subject_display := NULL;
                ELSIF clean_path = 'requester' THEN
                    updated_requester := NULL;
                ELSIF clean_path = 'requesterDisplay' THEN
                    updated_requester_display := NULL;
                ELSIF clean_path = 'performer' THEN
                    updated_performer := NULL;
                ELSIF clean_path = 'performerDisplay' THEN
                    updated_performer_display := NULL;
                ELSE
            ...
            WHEN 'add' THEN
                IF clean_path = 'modality' THEN
                    IF updated_modality IS NULL THEN
                        updated_modality := value;
                    ELSE
                        updated_modality := updated_modality || value;
                    END IF;
                ELSIF clean_path = 'identifier' THEN
                ...
                ELSIF clean_path = 'code' THEN
                    IF updated_code IS NULL THEN
                        updated_code := value;
                    ELSE
                        updated_code := updated_code || value;
                    END IF;
                ELSIF clean_path = 'reason' THEN
                    IF updated_reason IS NULL THEN
                        updated_reason := value;
                    ELSE
                        updated_reason := updated_reason || value;
                    END IF;
                ELSIF clean_path = 'locationCode' AND updated_location IS NULL THEN
                    updated_location := op->>'value';
                ELSIF clean_path = 'status' AND updated_status IS NULL THEN
                    updated_status := (op->>'value')::status;
                ELSIF clean_path = 'priority' AND updated_priority IS NULL THEN
                    updated_priority := (op->>'value')::priority;
                ELSIF clean_path = 'insurance' AND updated_insurance IS NULL THEN
                    updated_insurance := op->>'value';
                ELSIF clean_path = 'occurrenceDateTime' AND updated_occurrence_datetime IS NULL THEN
                    updated_occurrence_datetime := (op->>'value')::timestamp;
                ELSIF clean_path = 'isExported' THEN
                    updated_is_exported := value;
                ELSIF clean_path = 'note' THEN
        ...
        RAISE NOTICE 'Updated after op %:
            ...
            updated_note,
            updated_subject,
            updated_subject_display,
            updated_requester,
            updated_requester_display,
            updated_performer,
            updated_performer_display;
    END LOOP;
